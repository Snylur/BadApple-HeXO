...
FPS = 30
valids = []
can = lambda x: all([1<=_[0]-(_[1]-1)//2<38 and 1<=_[1]<42 for _ in x])
pixels = [(p + (y-1)//2, y) for p in range(1, 38) for y in range(1, 42)]
for y in range(1, 42):
    for p in range(1, 38):
        x = p + (y-1)//2
        j = 0
...
for i in range(1, 6573): # 6573
    ...
    available = [_ for _ in valids]
    hexing = lambda a, v: a[v[0]]==a[v[1]]==a[v[2]]==a[v[3]]==a[v[4]]==a[v[5]];
    while True:
        assignment = {_: int(_ in l) for _ in pixels}
        while True:
            bad = {}
            flag = False
            for valid in available:
                if hexing(assignment, valid):
                    flag += True
                    for _ in valid:
                        if _ not in bad: bad[_] = 0
                        bad[_] += 1
            if not flag: break
            those = list(bad.keys())
            shuff(those)
            take = max(those, key=lambda x: bad[x])
            assignment[take] ^= 1
            if flag == 1: break
            if flag == 0: break
        k = choose(available)
    ...
    pygame.image.save(window, f"new-idea-frames/output_{stri}.jpg")
    clock.tick(FPS)
